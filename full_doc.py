...
#####################################################
### PROGRAM IMPORTS
from typing import List, Set, Dict, Optional, Callable, TypeVar
...
# Own Modules
from summary import DEFAULT_TREE_SUMMARY_TEMPLATE, DEFAULT_ONELINE_SUMMARY_TEMPLATE
...
class FullDocument():
    ...
    retriever: BaseRetriever  # TODO: Consider multiple retrievers for keywords vs semantic.
    engine: BaseQueryEngine  # TODO: Consider mulitple engines. 
    ...
    def file_to_nodes(
        self, 
        reader: BaseReader,
        postreaders: Optional[List[Callable[[List[GenericNode]], List[GenericNode]] | TransformComponent]]=None,  # NOTE: these should be used in order. and probably all TransformComponent instead.
        node_parser: Optional[NodeParser]=None,
        postparsers: Optional[List[Callable[[List[GenericNode]], List[GenericNode]] | TransformComponent]]=None,  # Stuff like chunking, adding Embeddings, etc.
    ) -> None:
        ...
        if (node_parser is None and Settings.node_parser is not None):
    ...
    def nodes_to_summary(
        ...
        if (not hasattr(self, "nodes")):
            raise ValueError("Nodes must be extracted from document using `file_to_nodes` before calling `nodes_to_summary`.")
        ...
        if (summary is None or not isinstance(summary, str)):
            # TODO: ... this should always give us a string, right? we're not doing anything fancy with TokenGen/TokenAsyncGen/Pydantic BaseModel...
            raise ValueError(f"Summarizer must return a string summary. Actual type: {type(summary)}, with value {summary}.")
    ...
    def summary_to_oneline(
        ...
        if (not hasattr(self, "summary")):
            raise ValueError("Summary must be extracted from document using `nodes_to_summary` before calling `summary_to_oneline`.")
    ...
    def nodes_to_document_keywords(self, keyword_extractor: Optional[KeywordMetadataAdder] = None) -> None:
        ...
        """
        if (not hasattr(self, "nodes")):
            raise ValueError("Nodes must be extracted from document using `file_to_nodes` before calling `nodes_to_keywords`.")
        ...
        # TODO: handle dedupling keywords which are similar to each other (fuzzy?)
    ...
    def nodes_to_storage(self, create_new_storage: bool = True) -> None:
        ...
        if (not hasattr(self, "nodes")):
            raise ValueError("Nodes must be extracted from document using `file_to_nodes` before calling `nodes_to_storage`.")
        ...
        if (create_new_storage):
            ...
            raise NotImplementedError("Currently creates new storage for every document.")  ### TODO: use an existing storage instead of creating a new one.
    ...
    def retriever_to_engine(self, response_synthesizer: BaseSynthesizer, callback_manager: Optional[CallbackManager]=None) -> None:
        """Create query engine from retriever."""
        if (not hasattr(self, 'retriever')):
            raise ValueError("Retriever must be extracted from document using `storage_to_retriever` before calling `retriver_to_engine`.")
    ...
    # TODO: Create Summarization Index and Engine.
    def engine_to_sub_question_engine(self) -> None:
        ...
        if (not hasattr(self, 'summary_oneline')):
            raise ValueError("One Line Summary must be created for the document before calling `engine_to_sub_query_engine`")
        elif (not hasattr(self, 'engine')):
            raise ValueError("Basic Query Engine must be created before calling `engine_to_sub_query_engine`")
        
        sqe_tools = [
            QueryEngineTool(
                query_engine=self.engine,  # TODO: handle mulitple engines?
